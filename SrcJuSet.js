            ...
            if(input=="搜视口令"){
                ...
                return `fileSelect://`+$.toString(()=>{
                    if(/JYshare_/.test(input) && input.endsWith('txt')){
                        require(config.依赖.match(/http(s)?:\/\/.*\//)[0] + 'SrcJuSet.js');
                        input = '搜视接口￥' + aesEncode('SrcJu', input) + '￥文件导入';
                        return JYimport(input);
...
function jiekouapi(sourcefile, data, look) {
    ...
    if (data&&getMyVar('SrcJu_jiekouedit')!="1") {
        ...
        putMyVar('SrcJu_jiekoutype', data.type||"影视");
    ...
    d.push({
        title: '接口类型：'+ getMyVar('SrcJu_jiekoutype',getMyVar("SrcJu_jiekouType") == "全部"?'影视':getMyVar("SrcJu_jiekouType")),
        col_type: 'text_1',
    ...
    if(!look){
        d.push({
            ...
            url: $(getItem('searchtestkey', '斗罗大陆'),"输入测试搜索关键字").input(()=>{
                ...
                let type = getMyVar('SrcJu_jiekoutype','影视');
        ...
        d.push({
            ...
            url: $().lazyRule((sourcefile,oldtype,runModes) => {
                ...
                try {
                    ...
                    let type = getMyVar('SrcJu_jiekoutype',getMyVar("SrcJu_jiekouType") == "全部"?'影视':getMyVar("SrcJu_jiekouType"));
